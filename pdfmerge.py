...
def sanitize_file_path(output_dir, output_file_name):
    '''Get rid of \, and make sure there are slashes between and .pdf at the end of the file name. Returns the sanitized file path.'''
...
def merge_two_pdfs(input1, input2, output_dir, output_file_name="mergedpdf"):
    '''Simply merges two given PDFs together, with input1 first and input2 second. The PDF will be sent to the output directory (output_dir), which has a default name "mergedpdf.pdf".'''
    ...
    # Check that the output folder exists, otherwise create a new folder. In merge_two_pdfs, this is done later than checking the inputs as there are only two inputs. We can then avoid creating a new directory where there is no need for one.
