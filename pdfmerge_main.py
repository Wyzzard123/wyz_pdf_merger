...
def folder_merge():
    ...
    if output_folder.get() == "":
        messagebox.showerror("Error: No Output Folder specified",
                             '''Choose a folder where your new PDF will be located. Try not to copy paste directly 
                             into this box if you are using Windows, as the backslashes may lead to issues. Use the 
                             'Output' box instead.''')
        return
    ...
    try:
        destination, errors = pdfmerge.merge_pdfs_from_folder(input_folder.get(), output_folder.get(),
                                                              output_file.get())
    except FileNotFoundError:
        messagebox.showerror("Error: File Not Found",
                             f'''File {input_folder.get()} does not exist. Are there backslashes in the address? If 
                             so, use the Output button instead of copy pasting a file address directly.''')
        return
    ...
    merge_from_folder_button.configure(state=tk.NORMAL)
...
# Output Button
output_button = ttk.Button(text="Output", command=lambda: choose_folder_button(output_folder))
...
# Input Button
input_button = ttk.Button(text="Input", command=lambda: choose_folder_button(input_folder))
input_button.grid(row=5, column=1)

# Merge all in folder
merge_from_folder_button = ttk.Button(text="Merge from Input Folder", command=folder_merge)
merge_from_folder_button.grid(row=6, column=0)
